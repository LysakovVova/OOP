...
public class Game {
    ...
    void nextStep() {
        ...
        if (st == GameState.USER_MOVE) userMode();
        if (st == GameState.DILLER_MOVE) dillerMode();
        if (st == GameState.END_GAME) endGame();
        if (st == GameState.START_GAME) newGame();
        if (st == GameState.STOP_GAME) stopGame();
    }
}
